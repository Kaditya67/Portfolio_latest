...
export const api = {
  ...
  getProjects: () => apiCall("/projects/admin/all"),
    ...
    createProject: (data) => apiCall("/projects", { method: "POST", body: JSON.stringify(data) }),
    updateProject: (slug, data) =>
      apiCall(`/projects/${encodeURIComponent(slug)}`, { method: "PUT", body: JSON.stringify(data) }),
    deleteProject: (slug) => apiCall(`/projects/${encodeURIComponent(slug)}`, { method: "DELETE" }),
